...
The scripts generate several files during setup and deployment. These are typically **not committed** to your repo (they're in `.gitignore`):

**Generated by `cloudrun_ci_cd.sh`:**
- `Makefile` - Common Cloud Run targets (deploy, logs, manage, etc.)
- `.github/workflows/deploy-to-cloudrun.yml` - GitHub Actions workflow
- `cloudbuild.yaml` - Cloud Build configuration
...
**Generated by `deploy_to_cloudrun.sh`:**
- `Dockerfile` - If you choose to generate one for your stack
- `.cloudrun_deploy_<service>.conf` - Deployment configuration cache

**Generated by `setup_gcp_environment.sh`:**
- `.gcp_cloudrun_config` - Environment setup cache
...
- **May contain sensitive info** (project IDs, service accounts)
- **Easy to regenerate** by running the scripts

### When to Commit Them

You **should commit** if:
- You want to version-control your CI/CD config (cloudbuild.yaml, workflows)
- You have a custom Dockerfile specific to your app
- You want to track Makefile targets

In that case, remove them from `.gitignore` and commit selectively.


## Laravel-Specific Notes

When deploying Laravel apps:
...
APP_DEBUG=false
DB_CONNECTION=mysql
DB_HOST=  # Cloud SQL unix socket or IP
DB_PORT=3306
DB_DATABASE=your_db
DB_USERNAME=your_user
DB_PASSWORD=  # Use Secret Manager: --set-secrets="DB_PASSWORD=db-pass:latest"
```

**Cloud SQL Connection:**
```bash
# Use the unix socket path (recommended)
DB_HOST=/cloudsql/PROJECT:REGION:INSTANCE

# And add Cloud SQL flag during deployment:
--add-cloudsql-instances=PROJECT:REGION:INSTANCE
```

**Storage & Sessions:**
- Use `SESSION_DRIVER=cookie` or `redis` (not `file` in production)
- Use `CACHE_DRIVER=redis` or `database` (Cloud Memorystore or Cloud SQL)
- Laravel's `storage/` is ephemeral; use Cloud Storage for uploads

**Migrations:**
- Don't run migrations in the container startup
- Run as a separate Cloud Build step or Cloud Run Job:
```bash
...
When deploying Node.js TypeScript apps:
...
```dockerfile
# If output is in build/ instead of dist/
...
- Build fails: Check `tsc` runs locally first
- Wrong entry point: Verify `CMD` in Dockerfile matches your build output
